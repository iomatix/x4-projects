...
to reduce file sizes.
    ...
    function L.Raise_Signal(name, value)
        if isDebug then
            DebugError("[Hotkey.Interface] Raise_Signal: Signalling " .. tostring(name) .. " with value: " ..
                           tostring(value))
        end -- Debug: Log signal trigger
        AddUITriggeredEvent("Hotkey", name, value)
    end

    function L.Init()
        if isDebug then
            DebugError("[Hotkey.Interface] Init: Starting initialization")
        end -- Debug: Log initialization start
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] Init: Registered directtextinput event")
        end -- Debug: Log directtextinput registration

        -- MD triggered events.
        RegisterEvent("Hotkey.Update_Actions", L.Update_Actions)
        if isDebug then
            DebugError("[Hotkey.Interface] Init: Registered Hotkey.Update_Actions event")
        end -- Debug: Log Update_Actions registration
        RegisterEvent("Hotkey.Update_Player_Keys", L.Read_Player_Keys)
        if isDebug then
            DebugError("[Hotkey.Interface] Init: Registered Hotkey.Update_Player_Keys event")
        end -- Debug: Log Update_Player_Keys registration
        RegisterEvent("Hotkey.Update_Connection_Status", L.Update_Connection_Status)
        if isDebug then
            DebugError("[Hotkey.Interface] Init: Registered Hotkey.Update_Connection_Status event")
        end -- Debug: Log Update_Connection_Status registration
        RegisterEvent("Hotkey.Process_Message", L.Process_Message)
        if isDebug then
            DebugError("[Hotkey.Interface] Init: Registered Hotkey.Process_Message event")
        end -- Debug: Log Process_Message registration

        -- Cache the player component id.
        L.player_id = ConvertStringTo64Bit(tostring(C.GetPlayerID()))
        if isDebug then
            DebugError("[Hotkey.Interface] Init: Cached player ID: " .. tostring(L.player_id))
        end -- Debug: Log player ID caching
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] Init: Signalled reloaded")
        end -- Debug: Log reloaded signal
        ...
        L.Init_Menu()
        if isDebug then
            DebugError("[Hotkey.Interface] Init: Skipped Init_Menu (disabled)")
        end -- Debug: Log Init_Menu skip
    end
    ...
    function L.onEvent_directtextinput(some_userdata, char_entered)
        if isDebug then
            DebugError("[Hotkey.Interface] onEvent_directtextinput: Caught directtextinput, char: " .. tostring(char_entered))
        end -- Debug: Log directtextinput event
        ...
        if not L.alarm_pending then
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] onEvent_directtextinput: Hotkeys disabled")
            end -- Debug: Log disable signal and alarm state
        end

        -- Start or reset the timer.
        Time.Set_Alarm(L.alarm_id, L.direct_input_suppress_time, L.Reenable_Hotkeys)
        if isDebug then
            DebugError("[Hotkey.Interface] onEvent_directtextinput: Set alarm " .. tostring(L.alarm_id) .. " for " ..
                           tostring(L.direct_input_suppress_time) .. " seconds")
        end -- Debug: Log alarm setting
    end

    function L.Reenable_Hotkeys()
        -- if isDebug then DebugError("directtextinput event timed out; ending disable")
        if isDebug then
            DebugError("[Hotkey.Interface] Reenable_Hotkeys: Alarm timed out, reenabling hotkeys")
        end -- Debug: Log hotkey reenable
        L.Raise_Signal("enable")
        L.alarm_pending = false
        if isDebug then
            DebugError("[Hotkey.Interface] Reenable_Hotkeys: Signalled enable, set alarm_pending to false")
        end -- Debug: Log enable signal and alarm state
    end
    ...
    function L.Process_Message(_, message)
        if isDebug then
            DebugError("[Hotkey.Interface] Process_Message: Processing message: " .. tostring(message))
        end -- Debug: Log message processing
        -- Split it.
        local events = Lib.Split_String_Multi(message, ";")
        if isDebug then
            DebugError("[Hotkey.Interface] Process_Message: Split message into " .. tostring(#events) .. " events")
        end -- Debug: Log event splitting
        -- Add '$' prefixes and return.
        for i, event in ipairs(events) do
            events[i] = "$" .. event
            if isDebug then
                DebugError("[Hotkey.Interface] Process_Message: Prefixed event " .. tostring(i) .. ": " ..
                               tostring(events[i]))
            end -- Debug: Log event prefixing
        end
        -- Send back.
        L.Raise_Signal("handle_events", events)
        if isDebug then
            DebugError("[Hotkey.Interface] Process_Message: Signalled handle_events with " .. tostring(#events) ..
                           " events")
        end -- Debug: Log handle_events signal
    end

    -- Handle md pipe connection status update. Param is 0 or 1.
    function L.Update_Connection_Status(_, connected)
        if isDebug then
            DebugError("[Hotkey.Interface] Update_Connection_Status: Received connection status: " ..
                           tostring(connected))
        end -- Debug: Log connection status update
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] Update_Connection_Status: Set pipe_connected to " ..
                           tostring(L.pipe_connected))
        end -- Debug: Log pipe_connected state
    end
    ...
    function L.Update_Actions()
        if isDebug then
            DebugError("[Hotkey.Interface] Update_Actions: Reading action registry from blackboard")
        end -- Debug: Log action registry read
        -- Args are attached to the player component object.
        local md_table = GetNPCBlackboard(L.player_id, "$hotkey_api_actions")
        if isDebug then
            DebugError("[Hotkey.Interface] Update_Actions: Blackboard read result: " .. tostring(md_table))
        end -- Debug: Log blackboard read result
        ...
        if not md_table then
            if isDebug then
                DebugError("[Hotkey.Interface] Update_Actions: No action registry data found, skipping")
            end -- Debug: Log nil blackboard skip
            return
        end
        L.action_registry = md_table
        if isDebug then
            DebugError("[Hotkey.Interface] Update_Actions: Updated action_registry")
        end -- Debug: Log action registry update

        -- Clear the md var by writing nil.
        SetNPCBlackboard(L.player_id, "$hotkey_api_actions", nil)
        if isDebug then
            DebugError("[Hotkey.Interface] Update_Actions: Cleared $hotkey_api_actions blackboard")
        end -- Debug: Log blackboard clear
    ...
    function L.Read_Player_Keys()
        if isDebug then
            DebugError("[Hotkey.Interface] Read_Player_Keys: Reading player action keys from blackboard")
        end -- Debug: Log player keys read
        -- Args are attached to the player component object.
        local md_table = GetNPCBlackboard(L.player_id, "$hotkey_api_player_keys_from_md")
        if isDebug then
            DebugError("[Hotkey.Interface] Read_Player_Keys: Blackboard read result: " .. tostring(md_table))
        end -- Debug: Log blackboard read result
        -- This shouldn't get getting nil values since the md init is
        -- sent just once, but play it safe.
        if not md_table then
            if isDebug then
                DebugError("[Hotkey.Interface] Read_Player_Keys: No player keys data found, skipping")
            end -- Debug: Log nil blackboard skip
            return
        end
        L.player_action_keys = md_table
        if isDebug then
            DebugError("[Hotkey.Interface] Read_Player_Keys: Updated player_action_keys")
        end -- Debug: Log player keys update

        -- Clear the md var by writing nil.
        SetNPCBlackboard(L.player_id, "$hotkey_api_player_keys_from_md", nil)
        if isDebug then
            DebugError("[Hotkey.Interface] Read_Player_Keys: Cleared $hotkey_api_player_keys_from_md blackboard")
        end -- Debug: Log blackboard clear
    ...
    function L.Write_Player_Keys()
        if isDebug then
            DebugError("[Hotkey.Interface] Write_Player_Keys: Writing player action keys to blackboard")
        end -- Debug: Log player keys write
        -- Args are attached to the player component object.
        SetNPCBlackboard(L.player_id, "$hotkey_api_player_keys_from_lua", L.player_action_keys)
        if isDebug then
            DebugError("[Hotkey.Interface] Write_Player_Keys: Set $hotkey_api_player_keys_from_lua blackboard")
        end -- Debug: Log blackboard set
        L.Raise_Signal("Store_Player_Keys")
        if isDebug then
            DebugError("[Hotkey.Interface] Write_Player_Keys: Signalled Store_Player_Keys")
        end -- Debug: Log signal

        -- Lib.Print_Table(L.player_action_keys, "Write_Player_Keys player_action_keys")
    end

    -- Periodic check to see if any menus are open.
    ...
    function L.registerDirectInput(ego_registerDirectInput)
        if menu.remapControl and menu.remapControl.controlcontext == "hotkey_api" then
            ...
            for event, func in pairs(L.input_event_handlers) do
                ...
                if isDebug then
                    DebugError("[Hotkey.Interface] registerDirectInput: Registered event: " .. tostring(event))
                end
            end
            ListenForInput(true)
            if isDebug then
                DebugError("[Hotkey.Interface] registerDirectInput: Listening for keyboard input")
            end
        else
    ...
    -- Unregister direct input handler.
    function L.unregisterDirectInput(ego_unregisterDirectInput)
        if menu.remapControl and menu.remapControl.controlcontext == "hotkey_api" then
            ...
            for event, func in pairs(L.input_event_handlers) do
                ...
                if isDebug then
                    DebugError("[Hotkey.Interface] unregisterDirectInput: Unregistered event: " .. tostring(event))
                end
            end
            C.EnableAutoMouseEmulation()
            if isDebug then
                DebugError("[Hotkey.Interface] unregisterDirectInput: Stopped listening")
            end
        else
    ...
    function L.Init_Menu()
        if isDebug then
            DebugError("[Hotkey.Interface] Init_Menu: Starting menu initialization")
        end -- Debug: Log menu initialization start
        -- Look up the menu, store in this module's local.
        menu = Lib.Get_Egosoft_Menu("OptionsMenu")
        if isDebug then
            DebugError("[Hotkey.Interface] Init_Menu: Retrieved OptionsMenu")
        end -- Debug: Log menu retrieval

        -- Patch displayControls.
        local ego_displayControls = menu.displayControls("keyboard_space")
        if isDebug then
            DebugError("[Hotkey.Interface] Init_Menu: Patching displayControls")
        end -- Debug: Log displayControls patch

        menu.displayControls = function(...)
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Called with args: " .. tostring(select("#", ...)))
            end -- Debug: Log displayControls call
            ...
            menu.createOptionsFrame = function(...)
                if isDebug then
                    DebugError("[Hotkey.Interface] createOptionsFrame: Creating options frame")
                end -- Debug: Log frame creation
                -- Build the frame.
                frame = ego_createOptionsFrame(...)
                -- Record its display function. In 7.0 there's no frame.display method.            
                if frame.display then
                    ...
                    if isDebug then
                        DebugError("[Hotkey.Interface] createOptionsFrame: Set frame display to dummy function")
                    end -- Debug: Log display suppression
                else
                    if isDebug then
                        DebugError("[Hotkey.Interface] createOptionsFrame: Frame has no display function")
                    end -- Debug: Log missing display function
                end

                if isDebug then
                    DebugError("[Hotkey.Interface] createOptionsFrame: Suppressed frame display")
                end -- Debug: Log display suppression
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Recorded preselectOption: " .. tostring(preselectOption))
            end -- Debug: Log preselectOption
            -- Build the menu.
            ego_displayControls(...)
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Ran original displayControls")
            end -- Debug: Log original displayControls call

            -- Reconnect the createOptionsFrame function, to avoid impacting
            -- other menu pages.
            menu.createOptionsFrame = ego_createOptionsFrame
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Restored createOptionsFrame")
            end -- Debug: Log frame restoration
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Custom displayControls call success: " ..
                               tostring(success))
            end -- Debug: Log custom displayControls result
            ...
            if frame_display then
                ...
                frame:display()
                if isDebug then
                    DebugError("[Hotkey.Interface] displayControls: Restored and called frame display")
                end -- Debug: Log frame display
            elseif frame.display then
                frame:display()
                if isDebug then
                    DebugError("[Hotkey.Interface] displayControls: Called frame display without restoration")
                end -- Debug: Log frame display call
            end
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] Init_Menu: Patching remapInput")
        end -- Debug: Log remapInput patch
        menu.remapInput = function(...)
            if isDebug then
                DebugError("[Hotkey.Interface] remapInput: Called with args: " .. tostring(select("#", ...)))
            end -- Debug: Log remapInput call
            -- Hand off to ego function if this isn't a custom key.
            if menu.remapControl.controlcontext ~= "hotkey_api" then
                if isDebug then
                    DebugError("[Hotkey.Interface] remapInput: Non-hotkey_api context, calling original remapInput")
                end -- Debug: Log non-hotkey_api context
                ego_remapInput(...)
                return
            end
            -- Safety call.
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] remapInput: Custom remapInput call success: " .. tostring(success))
            end -- Debug: Log custom remapInput result
        end
        ...
        L.input_event_handlers = {
            ...
            ["keyboardInput"] = function(_, keycode)
                L.remapInput(1, keycode, 0)
            end
        }
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] Init_Menu: Skipped input event handler patches")
        end -- Debug: Log skipped input patches
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] Init_Menu: Patching Helper.clearMenu")
        end -- Debug: Log clearMenu patch
        Helper.clearMenu = function(menu, ...)
            if isDebug then
                DebugError("[Hotkey.Interface] clearMenu: Closing menu: " .. tostring(menu.name))
            end -- Debug: Log menu closing
        ...
        local function patch_showMenuCallback(menu)
            ...
            menu.showMenuCallback = function(...)
                if isDebug then
                    DebugError("[Hotkey.Interface] showMenuCallback: Opening menu: " .. tostring(menu.name))
                end -- Debug: Log menu opening
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] showMenuCallback: Patched showMenuCallback for menu: " ..
                               tostring(menu.name))
            end -- Debug: Log callback patch
        end
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] Init_Menu: Patched existing menus")
        end -- Debug: Log existing menu patches
        -- Patch future menus.
        local ego_registerMenu = Helper.registerMenu
        if isDebug then
            DebugError("[Hotkey.Interface] Init_Menu: Patching Helper.registerMenu")
        end -- Debug: Log registerMenu patch
        Helper.registerMenu = function(menu, ...)
            if isDebug then
                DebugError("[Hotkey.Interface] registerMenu: Registering menu: " .. tostring(menu.name))
            end -- Debug: Log menu registration
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] Init_Menu: Patching Helper.minimizeMenu")
        end -- Debug: Log minimizeMenu patch
        Helper.minimizeMenu = function(menu, ...)
            if isDebug then
                DebugError("[Hotkey.Interface] minimizeMenu: Minimizing menu: " .. tostring(menu.name))
            end -- Debug: Log menu minimizing
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] Init_Menu: Patching Helper.restoreMenu")
        end -- Debug: Log restoreMenu patch
        Helper.restoreMenu = function(menu, ...)
            if isDebug then
                DebugError("[Hotkey.Interface] restoreMenu: Restoring menu: " .. tostring(menu.name))
            end -- Debug: Log menu restoring
            ego_restoreMenu(menu, ...)
            L.Menu_Restored(menu)
        end

        if isDebug then
            DebugError("[Hotkey.Interface] Init_Menu: Completed menu initialization")
        end -- Debug: Log menu initialization complete

        AddUITriggeredEvent("Hotkey", "periodic_menu_check", nil)
        RegisterEvent("Hotkey.periodic_menu_check", function()
            L.Periodic_Menu_Check()
            Helper.setTimeout(5, function()
                AddUITriggeredEvent("Hotkey", "periodic_menu_check", nil)
            end)
        end)
        if isDebug then
            DebugError("[Hotkey.Interface] Init_Menu: Started periodic menu check")
        end
    end
    ...
    function L.Menu_Closed(menu)
        if menu.name == "TopLevelMenu" then
            if isDebug then
                DebugError("[Hotkey.Interface] Menu_Closed: Ignoring TopLevelMenu")
            end -- Debug: Log ignored menu
            return
        end
        -- DebugError("Menu closed: "..tostring(menu.name))
        if isDebug then
            DebugError("[Hotkey.Interface] Menu_Closed: Signalling Menu_Closed for: " .. tostring(menu.name))
        end -- Debug: Log menu close signal
    ...
    function L.Menu_Opened(menu)
        if menu.name == "TopLevelMenu" then
            if isDebug then
                DebugError("[Hotkey.Interface] Menu_Opened: Ignoring TopLevelMenu")
            end -- Debug: Log ignored menu
            return
        end
        if menu.name == "UserQuestionMenu" then
            if isDebug then
                DebugError("[Hotkey.Interface] Menu_Opened: Ignoring UserQuestionMenu")
            end -- Debug: Log ignored menu
            return
        end
        -- DebugError("Menu opened: "..tostring(menu.name))
        if isDebug then
            DebugError("[Hotkey.Interface] Menu_Opened: Signalling Menu_Opened for: " .. tostring(menu.name))
        end -- Debug: Log menu open signal
    ...
    function L.Menu_Minimized(menu)
        if menu.name == "TopLevelMenu" then
            if isDebug then
                DebugError("[Hotkey.Interface] Menu_Minimized: Ignoring TopLevelMenu")
            end -- Debug: Log ignored menu
            return
        end
        -- if isDebug then DebugError("Menu minimized: "..tostring(menu.name))
        if isDebug then
            DebugError("[Hotkey.Interface] Menu_Minimized: Signalling Menu_Closed for: " .. tostring(menu.name))
        end -- Debug: Log menu minimize signal
        L.Raise_Signal("Menu_Closed", menu.name)
    end
    ...
    function L.Menu_Restored(menu)
        if menu.name == "TopLevelMenu" then
            if isDebug then
                DebugError("[Hotkey.Interface] Menu_Restored: Ignoring TopLevelMenu")
            end -- Debug: Log ignored menu
            return
        end
        -- DebugError("Menu restored: "..tostring(menu.name))
        if isDebug then
            DebugError("[Hotkey.Interface] Menu_Restored: Signalling Menu_Opened for: " .. tostring(menu.name))
        end -- Debug: Log menu restore signal
        L.Raise_Signal("Menu_Opened", menu.name)
    end
...
L.Input_Listener()
    ...
    function L.displayControls(preselectOption, optionParameter)
        if isDebug then
            DebugError(
                "[Hotkey.Interface] displayControls: Called with preselectOption: " .. tostring(preselectOption) ..
                    ", optionParameter: " .. tostring(optionParameter))
        end -- Debug: Log function call
        ...
        if not L.pipe_connected then
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Pipe not connected, skipping")
            end -- Debug: Log pipe not connected
            return
        end
        ...
        if optionParameter ~= "keyboard_space" then
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Not keyboard_space, skipping")
            end -- Debug: Log non-keyboard_space skip
            return
        end
        ...
        if frame == nil then
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Failed to find optionsFrame")
            end -- Debug: Log frame not found
        ...
        for i = 1, #frame.content do
            if frame.content[i].type == "table" then
                ...
                if isDebug then
                    DebugError("[Hotkey.Interface] displayControls: Found table in frame content at index: " ..
                                   tostring(i))
                end -- Debug: Log table found
            end
        end
        if ftable == nil then
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Failed to find ftable")
            end -- Debug: Log table not found
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] displayControls: Action registry available, proceeding")
        end -- Debug: Log action registry check
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] displayControls: Added spacer row")
        end -- Debug: Log spacer row addition
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] displayControls: Added title row")
        end -- Debug: Log title row addition
        ...
        for _, action in pairs(L.action_registry) do
            if not L.player_action_keys[action.id] then
                ...
                if isDebug then
                    DebugError("[Hotkey.Interface] displayControls: Initialized player_action_keys for action ID: " ..
                                   tostring(action.id))
                end -- Debug: Log player keys initialization
            end
        end
        ...
        for _, action in pairs(L.action_registry) do
            ...
            if not cat_action_dict[cat] then
                cat_action_dict[cat] = {}
            end
            -- Add it in.
            table.insert(cat_action_dict[cat], action)
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Added action ID " .. tostring(action.id) ..
                               " to category: " .. tostring(cat))
            end -- Debug: Log action categorization
        end
        ...
        for cat, sublist in pairs(cat_action_dict) do
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Sorted actions for category: " .. tostring(cat))
            end -- Debug: Log action sorting
        end
        -- Sort the cat names.
        table.sort(cats_sorted)
        if isDebug then
            DebugError("[Hotkey.Interface] displayControls: Sorted categories: " ..
                           tostring(table.concat(cats_sorted, ",")))
        end -- Debug: Log category sorting
        ...
        for _, cat in ipairs(cats_sorted) do
            ...
            if cat ~= "" then
                ...
                if isDebug then
                    DebugError("[Hotkey.Interface] displayControls: Added category header for: " .. tostring(cat))
                end -- Debug: Log category header
            ...
            for _, action in ipairs(cat_action_dict[cat]) do
                ...
                if isDebug then
                    DebugError("[Hotkey.Interface] displayControls: Added row for action ID: " .. tostring(action.id))
                end -- Debug: Log action row addition
            end
        end
        ...
        for i = 1, #ftable.rows do
            if ftable.rows[i].index == preselectOption then
                ...
                if isDebug then
                    DebugError("[Hotkey.Interface] displayControls: Set selected row to: " .. tostring(preselectOption))
                end -- Debug: Log row selection
                break
            end
        end
    ...
    function L.displayControlRow(ftable, action_id)
        if isDebug then
            DebugError("[Hotkey.Interface] displayControlRow: Adding row for action ID: " .. tostring(action_id))
        end -- Debug: Log row addition
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] displayControlRow: Created row for action ID: " .. tostring(action_id))
        end -- Debug: Log row creation
        ...
        if row.index == menu.preselectOption then
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] displayControlRow: Set selected row: " .. tostring(row.index))
            end -- Debug: Log row selection
            if menu.preselectCol == 3 or menu.preselectCol == 4 then
                ftable:setSelectedCol(menu.preselectCol)
                if isDebug then
                    DebugError("[Hotkey.Interface] displayControlRow: Set selected column: " ..
                                   tostring(menu.preselectCol))
                end -- Debug: Log column selection
            end
        end
        ...
        if action.description then
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] displayControlRow: Set action title: " .. tostring(action.name) ..
                               ", description: " .. tostring(action.description))
            end -- Debug: Log title and description
        else
            if isDebug then
                DebugError("[Hotkey.Interface] displayControlRow: Set action title: " .. tostring(action.name) ..
                               ", no description")
            end -- Debug: Log title without description
        end
        ...
        for i = 1, 2 do
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] displayControlRow: Processing input " .. tostring(i) ..
                               " for action ID: " .. tostring(action_id))
            end -- Debug: Log input processing
            ...
            if info.source then
                ...
                if isDebug then
                    DebugError("[Hotkey.Interface] displayControlRow: Input " .. tostring(i) .. " keyname: " ..
                                   tostring(keyname) .. ", keyicon: " .. tostring(keyicon))
                end -- Debug: Log keyname and keyicon
            end
            ...
            local col = i + 2
            local button = row[col]:createButton({
                mouseOverText = action.description or ""
            })
            if isDebug then
                DebugError("[Hotkey.Interface] displayControlRow: Created button at column: " .. tostring(col))
            end -- Debug: Log button creation
            -- Set up the text label; this applies even without a keyname since
            -- it handles blinking _.
            button:setText( -- 'nameControl' handles label blinking when changing.
            function()
                return menu.nameControl(keyname, row.index, col)
            end, {
                color = Color.text_normal
            })
            if isDebug then
                DebugError("[Hotkey.Interface] displayControlRow: Set button text for column: " .. tostring(col))
            end -- Debug: Log button text
            -- Add the icon.
            if keyicon then
                button:setText2(keyicon, {
                    halign = "right"
                })
                if isDebug then
                    DebugError("[Hotkey.Interface] displayControlRow: Set button icon for column: " .. tostring(col))
                end -- Debug: Log button icon
            end

            -- Clicks will hand off to buttonControl.
            row[col].handlers.onClick = function()
                return menu.buttonControl(row.index, {"functions", action_id, info.source, info.code, info.signum, col,
                                                      false, "hotkey_api", false})
            end
            if isDebug then
                DebugError("[Hotkey.Interface] displayControlRow: Set onClick handler for column: " .. tostring(col))
            end -- Debug: Log click handler
        end
    end
...
{ row, col, controltype, controlcode, controlcontext, oldinputtype,
    ...
    function L.remapInput(...)
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] remapInput: Unregistered direct input")
        end -- Debug: Log input unregistration
        ...
        local return_func = function()
            ...
            if isDebug then
                DebugError(
                    "[Hotkey.Interface] remapInput: Set preselect - topRow: " .. tostring(menu.preselectTopRow) ..
                        ", option: " .. tostring(menu.preselectOption) .. ", col: " .. tostring(menu.preselectCol))
            end -- Debug: Log preselect settings
            menu.remapControl = nil
            if isDebug then
                DebugError("[Hotkey.Interface] remapInput: Cleared remapControl")
            end -- Debug: Log remapControl clear
            menu.submenuHandler(menu.currentOption)
            if isDebug then
                DebugError("[Hotkey.Interface] remapInput: Called submenuHandler with option: " ..
                               tostring(menu.currentOption))
            end -- Debug: Log submenuHandler call
        end
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] remapInput: Wrapped call success: " .. tostring(success))
        end -- Debug: Log wrapped call result
    end

    -- Inner part of remapInput, allowed to error.
    function L.remapInput_wrapped(return_func, newinputtype, newinputcode, newinputsgn)
        if isDebug then
            DebugError("[Hotkey.Interface] remapInput_wrapped: Processing input - type: " .. tostring(newinputtype) ..
                           ", code: " .. tostring(newinputcode) .. ", signum: " .. tostring(newinputsgn))
        end -- Debug: Log input processing
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] remapInput_wrapped: Looked up action_keys for ID: " ..
                           tostring(menu.remapControl.controlcode))
        end -- Debug: Log action keys lookup
        -- Error if not found.
        if not action_keys then
            if isDebug then
                DebugError("[Hotkey.Interface] remapInput_wrapped: No action_keys for ID: " ..
                               tostring(menu.remapControl.controlcode))
            end -- Debug: Log action keys not found
        ...
        if newinputtype ~= 1 then
            if isDebug then
                DebugError(
                    "[Hotkey.Interface] remapInput_wrapped: Non-keyboard input not supported, restarting input listener")
            end -- Debug: Log non-keyboard input
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] remapInput_wrapped: Selected input index: " .. tostring(input_index) ..
                           " for column: " .. tostring(menu.remapControl.col))
        end -- Debug: Log input index selection

        -- Note the prior key combo.
        local old_combo = action_keys.inputs[input_index].combo
        local new_combo
        if isDebug then
            DebugError("[Hotkey.Interface] remapInput_wrapped: Old combo: " .. tostring(old_combo))
        end -- Debug: Log old combo
        ...
        if newinputtype == 1 and newinputcode == 1 then
            if isDebug then
                DebugError("[Hotkey.Interface] remapInput_wrapped: Escape key detected, cancelling")
            end -- Debug: Log escape key
            -- Do nothing.
            return return_func()
        ...
        elseif newinputtype == 1 and newinputcode == 211 then
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] remapInput_wrapped: Delete key detected, resetting input")
            end -- Debug: Log delete key
        else
            -- Get the new combo string.
            new_combo = string.format("code %d %d %d", newinputtype, newinputcode, newinputsgn)
            if isDebug then
                DebugError("[Hotkey.Interface] remapInput_wrapped: New combo: " .. tostring(new_combo))
            end -- Debug: Log new combo
        ...
        if new_combo ~= "" and (action_keys.inputs[1].combo == new_combo or action_keys.inputs[2].combo == new_combo) then
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] remapInput_wrapped: Ignoring duplicate combo: " .. tostring(new_combo))
            end -- Debug: Log duplicate combo
            return return_func()
        end
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] remapInput_wrapped: Updated input " .. tostring(input_index) ..
                           " for action ID: " .. tostring(menu.remapControl.controlcode))
        end -- Debug: Log input update
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] remapInput_wrapped: Signalled Update_Key for action ID: " ..
                           tostring(action_keys.id))
        end -- Debug: Log Update_Key signal

        -- Update the md to save the keys.
        -- TODO: maybe integrate into Update_Key calls.
        L.Write_Player_Keys()
        if isDebug then
            DebugError("[Hotkey.Interface] remapInput_wrapped: Called Write_Player_Keys")
        end -- Debug: Log Write_Player_Keys call
...
Possibly adding new keys:
    ...
    if isDebug then
        DebugError("[Hotkey.Interface] Returning Init: " .. tostring(L.Init))
    end
    return nil, L.Init
end)
