...
- Assumes compatibility with `gameoptions.lua` input APIs; may need updates if new APIs are introduced.
    ...
    local L = {
        ...
        input_event_handlers = {
            ...
            ["gamepadInput"] = function(_, buttonid)
                L.remapInput(2, buttonid, 0)
            end,
            ["joystickInput"] = function(_, buttonid)
                L.remapInput(3, buttonid, 0)
            end
        },
    ...
    function L.Init()
        local isDebug = false -- Set to false for production
        if isDebug then
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] Init: Initialization complete")
        end
    end
    ...
    function L.onEvent_directtextinput(some_userdata, char_entered)
        if isDebug then
            DebugError("[Hotkey.Interface] onEvent_directtextinput: Caught char: " .. tostring(char_entered))
        end
    ...
    function L.Reenable_Hotkeys()
        if isDebug then
            DebugError("[Hotkey.Interface] Reenable_Hotkeys: Reenabling hotkeys")
        end
    ...
    function L.Update_Actions(_, action_list)
        if isDebug then
            DebugError("[Hotkey.Interface] Update_Actions: Updating action registry")
        end
        ...
        if menu and menu.optionsTable then
            menu.displayControls()
        end
        -- Debug: Log action registry update complete
        if isDebug then
            DebugError("[Hotkey.Interface] Update_Actions: Action registry updated with " .. tostring(#action_list) ..
                           " actions")
        end
    end
    ...
    function L.Read_Player_Keys(_, action_list)
        if isDebug then
            DebugError("[Hotkey.Interface] Read_Player_Keys: Reading player key bindings")
        end
        ...
        if menu and menu.optionsTable then
            ...
            DebugError(
                "[Hotkey.Interface] Read_Player_Keys: Skipping displayControls due to missing menu or optionsTable")
        end
        -- Debug: Log key binding read complete
        if isDebug then
            DebugError("[Hotkey.Interface] Read_Player_Keys: Read bindings for " .. tostring(#action_list) .. " actions")
        end
    end
    ...
    function L.Update_Connection_Status(_, connected)
        if isDebug then
            DebugError("[Hotkey.Interface] Update_Connection_Status: Pipe connected = " .. tostring(connected))
        end
    ...
    function L.Process_Message(_, message)
        if isDebug then
            DebugError("[Hotkey.Interface] Process_Message: Processing message")
        end
    ...
    function L.remapInput(inputtype, inputindex, modifier)
        if isDebug then
            DebugError("[Hotkey.Interface] remapInput: inputtype=" .. inputtype .. ", inputindex=" .. inputindex ..
                           ", modifier=" .. modifier)
        end
            ...
            if L.action_registry[actionid] then
                ...
                if inputname then
                    ...
                    if not blackboard then
                        ...
                        if isDebug then
                            DebugError("[Hotkey.Interface] remapInput: Initialized blackboard")
                        end
                    end
                    blackboard[actionid] = blackboard[actionid] or {}
                    blackboard[actionid][slot] = {
                        inputtype = inputtype,
                    ...
                    if isDebug then
                        DebugError("[Hotkey.Interface] remapInput: Bound action " .. actionid .. " to inputtype=" ..
                                       inputtype .. ", inputindex=" .. inputindex .. ", slot=" .. slot)
                    end
                else
                    DebugError("[Hotkey.Interface] remapInput: Invalid input for type=" .. inputtype .. ", index=" ..
                                   inputindex)
                    return
                end
            else
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] remapInput: Remapping complete")
        end
    end
    ...
    function L.displayControls(preselectOption, ...)
        if isDebug then
            DebugError("[Hotkey.Interface] displayControls: Adding custom hotkey rows")
        end
        ...
        if not keyboard_space then
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Created new keyboard_space section")
            end
        end
        ...
        for actionid, action in pairs(L.action_registry or {}) do
            ...
            local primary = bindings[1] or {
                inputtype = 1,
                inputindex = 0,
                modifier = 0
            }
            local secondary = bindings[2] or {
                inputtype = 1,
                inputindex = 0,
                modifier = 0
            }

            local primary_name = primary.inputindex > 0 and
                                     (C.GetInputName(primary.inputtype, primary.inputindex) or "") or ""
            local secondary_name = secondary.inputindex > 0 and
                                       (C.GetInputName(secondary.inputtype, secondary.inputindex) or "") or ""

            keyboard_space[2][#keyboard_space[2] + 1] = {{
                arrow = true
            }, {
                action.name or actionid,
                fontsize = Helper.standardFontSize
            }, {
                "",
                onClick = function()
                    menu.buttonControl(actionid, 1)
                end,
                displayValue = primary_name
            }, {
                "",
                onClick = function()
                    menu.buttonControl(actionid, 2)
                end,
                displayValue = secondary_name
            }}
            -- Debug: Log added hotkey row
            if isDebug then
                DebugError("[Hotkey.Interface] displayControls: Added row for action " .. tostring(actionid))
            end
        end
    end
    ...
    function L.registerDirectInput(ego_registerDirectInput)
        if menu.remapControl and menu.remapControl.controlcontext == "hotkey_api" then
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] registerDirectInput: Listening for all input types")
            end
        else
    ...
    function L.Init_Menu()
        ...
        L.ego_displayControls = ego_displayControls -- Store for later use
        ...
        Helper.clearMenu = function(m, ...)
            ego_helper_clearMenu(m, ...)
            L.Menu_Closed(m)
        end
        for _, m in ipairs(Menus) do
            local ego_show = m.showMenuCallback
            m.showMenuCallback = function(...)
                ego_show(...)
                L.Menu_Opened(m)
            end
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] Init_Menu: Registered show event for menu " .. m.name)
            end
        end
        ...
        Helper.registerMenu = function(m, ...)
            ...
            if isDebug then
                DebugError("[Hotkey.Interface] Init_Menu: Registered new menu " .. m.name)
            end
        end
        local ego_minimizeMenu = Helper.minimizeMenu
        Helper.minimizeMenu = function(m, ...)
            ego_minimizeMenu(m, ...)
            L.Menu_Minimized(m)
        end
        local ego_restoreMenu = Helper.restoreMenu
        Helper.restoreMenu = function(m, ...)
            ego_restoreMenu(m, ...)
            L.Menu_Restored(m)
        end
    ...
    function L.Periodic_Menu_Check()
        if isDebug then
            DebugError("[Hotkey.Interface] Periodic_Menu_Check: Checking menu states")
        end
        ...
        if isDebug then
            DebugError("[Hotkey.Interface] Periodic_Menu_Check: Menu open = " .. tostring(any_menu_open))
        end
    end
    ...
    function L.Menu_Opened(menu)
        if isDebug then
            DebugError("[Hotkey.Interface] Menu_Opened: Menu " .. tostring(menu.name) .. " opened")
        end
        -- Debug: Log menu open
    ...
    function L.Menu_Closed(menu)
        if isDebug then
            DebugError("[Hotkey.Interface] Menu_Closed: Menu " .. tostring(menu.name) .. " closed")
        end
    ...
    function L.Menu_Minimized(menu)
        if isDebug then
            DebugError("[Hotkey.Interface] Menu_Minimized: Menu " .. tostring(menu.name) .. " minimized")
        end
    ...
    function L.Menu_Restored(menu)
        if isDebug then
            DebugError("[Hotkey.Interface] Menu_Restored: Menu " .. tostring(menu.name) .. " restored")
        end
