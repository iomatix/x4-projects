...
Lua_Loader.define("extensions.sn_mod_support_apis.ui.named_pipes.Pipes", function(require)
  ...
  local function cleanup(p)
    if p.write_file then p.write_file:close_pipe() end
    if p.read_file  then p.read_file:close_pipe()  end
  end
  ...
  local function attach_gc(p)
    ...
    getmetatable(proxy).__gc = function() cleanup(p) end
  end
